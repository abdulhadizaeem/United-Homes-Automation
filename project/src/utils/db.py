...
def get_admin_calendar_credentials():
    try:
        ...
        logging.warning(
            "[DB] get_admin_calendar_credentials DB failed (%s); falling back to local cache file.",
            db_err,
        )
...
def save_admin_calendar_credentials(provider, email, credentials_dict):
    ...
    try:
        ...
        logging.warning("[DB] save_admin_calendar_credentials DB failed: %s", db_err)
...
def _update_appointment_field(appointment_id, field, value):
    ...
    try:
        cur.execute(
            f"UPDATE appointments SET {field} = %s, updated_at = CURRENT_TIMESTAMP "
            f"WHERE id = %s",
